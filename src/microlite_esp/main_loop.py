...
async def main_loop():
    ...
    esp_manager = ESPNowCommunicationManager(app_manager.get_peer_mac(), app_manager.get_network_interface(), True)
    ...
    while True:
        ...
        if not app_manager.sta_if.isconnected(): # TODO: To function
            ...
            app_manager.sta_if.active(True)
            while not app_manager.sta_if.active():
                time.sleep(0.1)
            app_manager.sta_if.config(channel=1)
        ...
        if msg_type == "PREDICT":
            ...
            if model_manager.active_model_name == None:
                ...
                await esp_manager.asend_text("Model not loaded")
                continue
            ...
            if not ok:
                ...
                await esp_manager.asend_message("PREDICT_FAIL")
                await esp_manager.asend_text("Not enough space on the device to save file")
                continue
            ...
            try:
                ...
                await esp_manager.asend_message("PREDICT_FAIL")
                await esp_manager.asend_text("Device is busy")
                continue
